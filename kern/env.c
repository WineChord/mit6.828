...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	for (size_t i = start; i < end; i += PGSIZE) {
		...
		int r = page_insert(e->env_pgdir, pp, (void *)i, PTE_U | PTE_W);
		if (r != 0)
			panic("page_insert: %e\n", r);
	}
}
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	// switch pgdir so that va translation of user address space
	// can take effect.
	uint32_t cr3 = rcr3();
...
env_create(uint8_t *binary, enum EnvType type)
{
	...
	// Answer to Question 1: there is no need to do anything further
	// when switching environments. Because eflags is saved and restored.
	if(type == ENV_TYPE_FS)
